...
BUILD_FLAGS=-DOPENSSL_USE_STATIC_LIBS=1 -DEXTENSION_STATIC_BUILD=1 -DBUILD_TPCH_EXTENSION=1 -DBUILD_PARQUET_EXTENSION=1 ${OSX_BUILD_UNIVERSAL_FLAG} ${STATIC_LIBCPP}

CLIENT_FLAGS :=

# These flags will make DuckDB build the extension
EXTENSION_FLAGS=-DDUCKDB_OOT_EXTENSION_NAMES="aws" -DDUCKDB_OOT_EXTENSION_AWS_PATH="$(PROJ_DIR)" -DDUCKDB_OOT_EXTENSION_AWS_SHOULD_LINK="TRUE" -DDUCKDB_OOT_EXTENSION_AWS_INCLUDE_PATH="$(PROJ_DIR)src/include"
...
debug:
	mkdir -p  build/debug && \
	cmake $(GENERATOR) $(FORCE_COLOR) $(EXTENSION_FLAGS) ${CLIENT_FLAGS} -DBUILD_HTTPFS_EXTENSION=1 -DEXTENSION_STATIC_BUILD=1 -DCMAKE_BUILD_TYPE=Debug ${BUILD_FLAGS} -S ./duckdb/ -B build/debug && \
	cmake --build build/debug --config Debug

release:
	mkdir -p build/release && \
	cmake $(GENERATOR) $(FORCE_COLOR) $(EXTENSION_FLAGS) ${CLIENT_FLAGS} -DBUILD_HTTPFS_EXTENSION=1 -DEXTENSION_STATIC_BUILD=1 -DCMAKE_BUILD_TYPE=Release ${BUILD_FLAGS} -S ./duckdb/ -B build/release && \
